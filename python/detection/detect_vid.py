...
def procesar_caracter(caracter):
    if caracter == "a":
        # Si el carácter es "a", imprimimos un mensaje en pantalla.
        print("Recibimos una 'a'!")
    elif caracter == "b":
        # Si el carácter es "b", imprimimos otro mensaje en pantalla.
        print("Recibimos una 'b'!")
    # Agrega más condiciones aquí para procesar otros caracteres.
...
try:
    ...
    print("No se detecto puerto serie conectado.")
    serialPort = None

# load the COCO class names
with open('python/model/object_detection_classes_coco.txt', 'r') as f:
...
# load the DNN model
model = cv2.dnn.readNet(model='python/model/frozen_inference_graph.pb',
                        config='python/model/ssd_mobilenet_v2_coco_2018_03_29.pbtxt.txt', 
...
#Para grabar salida descomentar bloque:
...
while True:
    #time.sleep(1)
    caracter = serialPort.read()
    if caracter == b"x":
        print("Iniciando detección...")
        detect_on = True

    if  caracter == b"y":
        print("Se realizó las detección!")
    ...
    if ret:
        ...
        if detect_on == True:
            ...
            start = time.time()
            model.setInput(blob) #Cargamos la imagen normalizada en el modelo
            output = model.forward() #Ejecutamos la detección
            # end time after detection
            end = time.time()
            # calculate the FPS for current frame detection
            fps = 1 / (end-start)

            # La detección nos devuelve un arreglo con datos por cada objeto que puede ser detectado (Lode la lista del txt)
            # junto con la probabilidad (confidence) y demas valores. Cunado la probabilidad es mayor que un umbral dado inferimos que tal objeto está en la imagen
            # Por cada deteccion en output
